// parameters for elements (dropzone5, elements)
const draggables = document.querySelectorAll('.draggable');
const dropzone5s = document.querySelectorAll('.dropzone5');
let selectedDropzone5 = null;
let dropzone5Count = 1; // Start with 1 because we have a default dropzone5
const MAX_DROPZONE5S = 3;
let labelCounter = 0;
...
// Resizable UI page elements
            ...
            .dropzone5 {
                position: relative;
                border-radius:20px;
                border:5px solid #333; /* Ensure buttons are positioned relative to the dropzone5 */
            }
           .resize-handle { 
                position: absolute; 
                bottom: 0; 
            ...
            .selected-dropzone5 { background-color: #f0f0f0; }
                  .resizable {
            position: relative;
...
dropzone5s.forEach(dropzone5 => {
    ...
    dropzone5.addEventListener('drop', function (event) {
        ...
        if (id && event.target.classList.contains('dropzone5')) {
        }
    });
    dropzone5.addEventListener('click', function () {
        if (selectedDropzone5) {
            selectedDropzone5.classList.remove('selected-dropzone5');
        }
        selectedDropzone5 = dropzone5;
        dropzone5.classList.add('selected-dropzone5');
    });
});
//dropzone5 eventlister ends here.......................................
// Add click event listener to the draggable elements
const draggableElements = document.querySelectorAll('.draggable');
draggableElements.forEach(element => {
    element.addEventListener('click', function () {
        if (selectedDropzone5) {
            const dropzone5Rect = selectedDropzone5.getBoundingClientRect();
            const dropzone5CenterX = dropzone5Rect.width / 2;
            const dropzone5CenterY = dropzone5Rect.height / 2;
            appendElementToDropzone5(element.id, selectedDropzone5, dropzone5CenterX, dropzone5CenterY);
        } else {
            alert('Please select a dropzone5 first.');
        }
    });
});
function appendElementToDropzone5(id, dropzone5, x, y) {
...
function appendElementToDropzone5(id, dropzone5, clientX, clientY) {
    let element;
    switch (id) {
       case 'H1':
        ...
        case 'text':
           ...
           ensureButtons();
            ...
            case 'select':
                ...
                addDeleteButton(element);
                element.style.left = `${clientX - dropzone5.getBoundingClientRect().left}px`;
                element.style.top = `${clientY - dropzone5.getBoundingClientRect().top}px`;
                addMoveButton(element);
                break;
        case 'image':
 ...
 //appending elements to UI page ends here...................................
  //not sure about which part
    ...
    const dropzone5Rect = dropzone5.getBoundingClientRect();
    const offsetX = clientX - dropzone5Rect.left;
    const offsetY = clientY - dropzone5Rect.top;
    if (id === 'image') {
    ...
    if (id === 'image') {
        ...
        element.addEventListener('drop', function (event) {
            event.preventDefault();
            const offsetX = event.clientX - dropzone5Rect.left;
            const offsetY = event.clientY - dropzone5Rect.top;
            element.style.left = `${offsetX}px`;
            element.style.top = `${offsetY}px`;
        });
...
//till here 
//UI page resize
function enableResize(dropzone5) {
    const resizeHandle = dropzone5.querySelector('.resize-handle');
    resizeHandle.addEventListener('mousedown', function (event) {
        event.preventDefault();
        event.stopPropagation();
        const initialHeight = dropzone5.offsetHeight;
        const initialY = event.clientY;
        function onMouseMove(event) {
            const height = initialHeight + (event.clientY - initialY);
            dropzone5.style.height = `${Math.max(50, height)}px`; // Set a minimum height of 50px
        }
        function onMouseUp() {
            document.removeEventListener('mousemove', onMouseMove);
            document.removeEventListener('mouseup', onMouseUp);
...
//Initialize default dropzone5 (when multiple dropzone5s were there)
function initializeDefaultDropzone5() {
    const defaultDropzone5 = document.getElementById('dropzone5');
    const resizeHandle = document.createElement('div');
    resizeHandle.classList.add('resize-handle');
    defaultDropzone5.appendChild(resizeHandle);
    enableResize(defaultDropzone5);
}
//Initialize default dropzone5 (when multiple dropzone5s were there) ends here..........................................
//share button handle function
function handleSharedDropzone5() {
    const urlParams = new URLSearchParams(window.location.search);
    const sharedDropzone5Id = urlParams.get('dropzone5');
    if (sharedDropzone5Id) {
        deserializeDropzone5(sharedDropzone5Id);
    }
}
// Call this function when the page loads
document.addEventListener('DOMContentLoaded', handleSharedDropzone5);
...
const dropzone5BgColorInput = document.getElementById('dropzone5BgColorInput');
...
applyButton.addEventListener('click', function () {
    if (selectedDropzone5) {
        ...
        const dropzone5BgColor = dropzone5BgColorInput.value;
        ...
        // Apply dropzone5 styles
        if (dropzone5BgColor) {
            selectedDropzone5.style.backgroundColor = dropzone5BgColor;
        }
        ...
        if (file) {
            ...
            reader.onload = function (e) {
                ...
                selectedDropzone5.style.backgroundImage = `url(${imageUrl})`;
                selectedDropzone5.style.backgroundSize = 'cover'; 
                selectedDropzone5.style.backgroundPosition = 'center'; 
            };
            reader.readAsDataURL(file);
        }

        // Apply selected element styles if an element is selected within the dropzone5
        const selectedElement = selectedDropzone5.querySelector('.dropped-element.selected');
        if (selectedElement) {
...
// Event listener for selecting elements within the dropzone5
...
function updateLabelDropdown() {
    ...
    const labels = document.querySelectorAll('.dropzone5 label');
...
function downloadTableImage() {
    ...
    if (!dropzone5) {
        console.error('Dropzone5 not found.');
        return;
    }

    html2canvas(dropzone5).then(canvas => {
        ...
        logo.onload = () => {
            ...
            link.href = imgData;
            link.download = 'dropzone5-image.png';
...
document.addEventListener('DOMContentLoaded', () => {
    restoreDropZone5();
    document.querySelector('.save-btn').addEventListener('click', freezeDropZone5);
    document.querySelector('.edit-btn').addEventListener('click', unfreezeDropZone5);
    document.querySelector('.share-btn').addEventListener('click', shareDropZone5);
    // Attach the delete function to the delete button
    document.querySelector('.delete-btn').addEventListener('click', clearDropzone5AndStorage);
});
function generateUniqueId() {
    return 'id-' + Math.random().toString(36).substr(2, 16);
}
function freezeDropZone5() {
    const dropzone5 = document.querySelector('.dropzone5');
    const elementsToSave = Array.from(dropzone5.children).map(element => ({
        id: element.id || generateUniqueId(),
        innerHTML: element.innerHTML,
        classList: Array.from(element.classList),
        style: element.getAttribute('style')
    }));
    localStorage.setItem('dropzone5BgColor', dropzone5.style.backgroundColor);
    localStorage.setItem('dropzone5BgImage', dropzone5.style.backgroundImage); // Save background image URL
    localStorage.setItem('dropzone5Contents', JSON.stringify(elementsToSave));
    Array.from(dropzone5.children).forEach(element => {
...
//edit button
function restoreDropZone5() {
    const dropzone5 = document.querySelector('.dropzone5');
    const savedElements = JSON.parse(localStorage.getItem('dropzone5Contents'));
    const savedBgColor = localStorage.getItem('dropzone5BgColor');
    const savedBgImage = localStorage.getItem('dropzone5BgImage'); // Get background image URL
    if (savedBgColor) {
        dropzone5.style.backgroundColor = savedBgColor;
    }
    if (savedBgImage) {
        dropzone5.style.backgroundImage = savedBgImage; // Apply background image URL
        dropzone5.style.backgroundSize = 'cover'; 
        dropzone5.style.backgroundPosition = 'center'; 
    }
    if (savedElements) {
        dropzone5.innerHTML = ''; // Clear existing content
...
// delete dropzone5
function clearDropzone5AndStorage() {
    const dropzone5 = document.querySelector('.dropzone5');
    if (dropzone5) {
        // Clear the contents of the dropzone5
        dropzone5.innerHTML = '';
        // Clear localStorage
        localStorage.removeItem('dropzone5BgColor');
        localStorage.removeItem('dropzone5Contents');
        localStorage.removeItem('dropzone5BgImage');
        console.log('Dropzone5 cleared and storage reset.');
    } else {
        console.log('Dropzone5 element not found.');
    }
}
// delete dropzone5 ends here.................................

//share button
function shareDropZone5() {
    const dropzone5Content = localStorage.getItem('dropzone5Contents');
    if (!dropzone5Content) {
        console.error('Nothing to share. Save the drop zone first.');
        return;
    }
    const dropzone5Id = 'unique-dropzone5-id';
    localStorage.setItem(dropzone5Id, dropzone5Content);
    const shareableLink = `${window.location.origin}${window.location.pathname}?dropzone5=${dropzone5Id}`;
    console.log('Shareable link:', shareableLink);
    alert(`Shareable link: ${shareableLink}`);
}
function serializeDropzone5(dropzone5) {
    const content = {
        style: dropzone5.getAttribute('style'),
        className: dropzone5.className,
        elements: []
    };
    dropzone5.querySelectorAll('.dropped-element').forEach(element => {
        let elementData = {
...
function generateUniqueId() {
    return 'dropzone5_' + Math.random().toString(36).substr(2, 9);
}
function reattachEventListeners(element) {
    element.addEventListener('dragstart', function (event) {
        event.dataTransfer.setData('text/plain', '');
...
function setCursorToEnd(element) {
     ...
     document.addEventListener('DOMContentLoaded', function() {
        ...
        dropzone5s.forEach(dropzone5 => {
            ...
            dropzone5.addEventListener('drop', function (event) {
                ...
                if (elementId && event.target.classList.contains('dropzone5')) {
                    ...
                    if (elementTitle) {
                        ...
                        // Append the new element to the dropzone5
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // Existing dropzone5 drop event logic
        const dropzone5 = document.querySelector('.dropzone5'); // Adjust selector as needed
        dropzone5.addEventListener('drop', function (event) {
            event.preventDefault();
            const id = event.dataTransfer.getData('text');
            if (id && event.target.classList.contains('dropzone5')) {
...
//delete button .......................
function clearDropzone5Contents() {
    document.getElementById('dropzone5Contents').innerHTML = '';
}
//sounds for elements......................
