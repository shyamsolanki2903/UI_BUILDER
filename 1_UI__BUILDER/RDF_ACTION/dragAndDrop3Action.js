// parameters for elements (dropzone3, elements)
const draggables = document.querySelectorAll('.draggable');
const dropzone3s = document.querySelectorAll('.dropzone3');
let selectedDropzone3 = null;
let dropzone3Count = 1; // Start with 1 because we have a default dropzone3
const MAX_DROPZONE3S = 3;
let labelCounter = 0;
...
// Resizable UI page elements
            ...
            .dropzone3 {
                position: relative;
                border-radius:20px;
                border:5px solid #333; /* Ensure buttons are positioned relative to the dropzone3 */
            }
           .resize-handle { 
                position: absolute; 
                bottom: 0; 
            ...
            .selected-dropzone3 { background-color: #f0f0f0; }
                  .resizable {
            position: relative;
...
dropzone3s.forEach(dropzone3 => {
    ...
    dropzone3.addEventListener('drop', function (event) {
        ...
        if (id && event.target.classList.contains('dropzone3')) {
        }
    });
    dropzone3.addEventListener('click', function () {
        if (selectedDropzone3) {
            selectedDropzone3.classList.remove('selected-dropzone3');
        }
        selectedDropzone3 = dropzone3;
        dropzone3.classList.add('selected-dropzone3');
    });
});
//dropzone3 eventlister ends here.......................................
// Add click event listener to the draggable elements
const draggableElements = document.querySelectorAll('.draggable');
draggableElements.forEach(element => {
    element.addEventListener('click', function () {
        if (selectedDropzone3) {
            const dropzone3Rect = selectedDropzone3.getBoundingClientRect();
            const dropzone3CenterX = dropzone3Rect.width / 2;
            const dropzone3CenterY = dropzone3Rect.height / 2;
            appendElementToDropzone3(element.id, selectedDropzone3, dropzone3CenterX, dropzone3CenterY);
        } else {
            alert('Please select a dropzone3 first.');
        }
    });
});
function appendElementToDropzone3(id, dropzone3, x, y) {
...
function appendElementToDropzone3(id, dropzone3, clientX, clientY) {
    let element;
    switch (id) {
       case 'H1':
        ...
        case 'text':
           ...
           ensureButtons();
            ...
            case 'select':
                ...
                addDeleteButton(element);
                element.style.left = `${clientX - dropzone3.getBoundingClientRect().left}px`;
                element.style.top = `${clientY - dropzone3.getBoundingClientRect().top}px`;
                addMoveButton(element);
                break;
        case 'image':
 ...
 //appending elements to UI page ends here...................................
  //not sure about which part
    ...
    const dropzone3Rect = dropzone3.getBoundingClientRect();
    const offsetX = clientX - dropzone3Rect.left;
    const offsetY = clientY - dropzone3Rect.top;
    if (id === 'image') {
    ...
    if (id === 'image') {
        ...
        element.addEventListener('drop', function (event) {
            event.preventDefault();
            const offsetX = event.clientX - dropzone3Rect.left;
            const offsetY = event.clientY - dropzone3Rect.top;
            element.style.left = `${offsetX}px`;
            element.style.top = `${offsetY}px`;
        });
...
//till here 
//UI page resize
function enableResize(dropzone3) {
    const resizeHandle = dropzone3.querySelector('.resize-handle');
    resizeHandle.addEventListener('mousedown', function (event) {
        event.preventDefault();
        event.stopPropagation();
        const initialHeight = dropzone3.offsetHeight;
        const initialY = event.clientY;
        function onMouseMove(event) {
            const height = initialHeight + (event.clientY - initialY);
            dropzone3.style.height = `${Math.max(50, height)}px`; // Set a minimum height of 50px
        }
        function onMouseUp() {
            document.removeEventListener('mousemove', onMouseMove);
            document.removeEventListener('mouseup', onMouseUp);
...
//Initialize default dropzone3 (when multiple dropzone3s were there)
function initializeDefaultDropzone3() {
    const defaultDropzone3 = document.getElementById('dropzone3');
    const resizeHandle = document.createElement('div');
    resizeHandle.classList.add('resize-handle');
    defaultDropzone3.appendChild(resizeHandle);
    enableResize(defaultDropzone3);
}
//Initialize default dropzone3 (when multiple dropzone3s were there) ends here..........................................
//share button handle function
function handleSharedDropzone3() {
    const urlParams = new URLSearchParams(window.location.search);
    const sharedDropzone3Id = urlParams.get('dropzone3');
    if (sharedDropzone3Id) {
        deserializeDropzone3(sharedDropzone3Id);
    }
}
// Call this function when the page loads
document.addEventListener('DOMContentLoaded', handleSharedDropzone3);
...
const dropzone3BgColorInput = document.getElementById('dropzone3BgColorInput');
...
applyButton.addEventListener('click', function () {
    if (selectedDropzone3) {
        ...
        const dropzone3BgColor = dropzone3BgColorInput.value;
        ...
        // Apply dropzone3 styles
        if (dropzone3BgColor) {
            selectedDropzone3.style.backgroundColor = dropzone3BgColor;
        }
        ...
        if (file) {
            ...
            reader.onload = function (e) {
                ...
                selectedDropzone3.style.backgroundImage = `url(${imageUrl})`;
                selectedDropzone3.style.backgroundSize = 'cover'; 
                selectedDropzone3.style.backgroundPosition = 'center'; 
            };
            reader.readAsDataURL(file);
        }

        // Apply selected element styles if an element is selected within the dropzone3
        const selectedElement = selectedDropzone3.querySelector('.dropped-element.selected');
        if (selectedElement) {
...
// Event listener for selecting elements within the dropzone3
...
function updateLabelDropdown() {
    ...
    const labels = document.querySelectorAll('.dropzone3 label');
...
function downloadTableImage() {
    ...
    if (!dropzone3) {
        console.error('Dropzone3 not found.');
        return;
    }

    html2canvas(dropzone3).then(canvas => {
        ...
        logo.onload = () => {
            ...
            link.href = imgData;
            link.download = 'dropzone3-image.png';
...
document.addEventListener('DOMContentLoaded', () => {
    restoreDropZone3();
    document.querySelector('.save-btn').addEventListener('click', freezeDropZone3);
    document.querySelector('.edit-btn').addEventListener('click', unfreezeDropZone3);
    document.querySelector('.share-btn').addEventListener('click', shareDropZone3);
    // Attach the delete function to the delete button
    document.querySelector('.delete-btn').addEventListener('click', clearDropzone3AndStorage);
});
function generateUniqueId() {
    return 'id-' + Math.random().toString(36).substr(2, 16);
}
function freezeDropZone3() {
    const dropzone3 = document.querySelector('.dropzone3');
    const elementsToSave = Array.from(dropzone3.children).map(element => ({
        id: element.id || generateUniqueId(),
        innerHTML: element.innerHTML,
        classList: Array.from(element.classList),
        style: element.getAttribute('style')
    }));
    localStorage.setItem('dropzone3BgColor', dropzone3.style.backgroundColor);
    localStorage.setItem('dropzone3BgImage', dropzone3.style.backgroundImage); // Save background image URL
    localStorage.setItem('dropzone3Contents', JSON.stringify(elementsToSave));
    Array.from(dropzone3.children).forEach(element => {
...
//edit button
function restoreDropZone3() {
    const dropzone3 = document.querySelector('.dropzone3');
    const savedElements = JSON.parse(localStorage.getItem('dropzone3Contents'));
    const savedBgColor = localStorage.getItem('dropzone3BgColor');
    const savedBgImage = localStorage.getItem('dropzone3BgImage'); // Get background image URL
    if (savedBgColor) {
        dropzone3.style.backgroundColor = savedBgColor;
    }
    if (savedBgImage) {
        dropzone3.style.backgroundImage = savedBgImage; // Apply background image URL
        dropzone3.style.backgroundSize = 'cover'; 
        dropzone3.style.backgroundPosition = 'center'; 
    }
    if (savedElements) {
        dropzone3.innerHTML = ''; // Clear existing content
...
// delete dropzone3
function clearDropzone3AndStorage() {
    const dropzone3 = document.querySelector('.dropzone3');
    if (dropzone3) {
        // Clear the contents of the dropzone3
        dropzone3.innerHTML = '';
        // Clear localStorage
        localStorage.removeItem('dropzone3BgColor');
        localStorage.removeItem('dropzone3Contents');
        localStorage.removeItem('dropzone3BgImage');
        console.log('Dropzone3 cleared and storage reset.');
    } else {
        console.log('Dropzone3 element not found.');
    }
}
// delete dropzone3 ends here.................................

//share button
function shareDropZone3() {
    const dropzone3Content = localStorage.getItem('dropzone3Contents');
    if (!dropzone3Content) {
        console.error('Nothing to share. Save the drop zone first.');
        return;
    }
    const dropzone3Id = 'unique-dropzone3-id';
    localStorage.setItem(dropzone3Id, dropzone3Content);
    const shareableLink = `${window.location.origin}${window.location.pathname}?dropzone3=${dropzone3Id}`;
    console.log('Shareable link:', shareableLink);
    alert(`Shareable link: ${shareableLink}`);
}
function serializeDropzone3(dropzone3) {
    const content = {
        style: dropzone3.getAttribute('style'),
        className: dropzone3.className,
        elements: []
    };
    dropzone3.querySelectorAll('.dropped-element').forEach(element => {
        let elementData = {
...
function generateUniqueId() {
    return 'dropzone3_' + Math.random().toString(36).substr(2, 9);
}
function reattachEventListeners(element) {
    element.addEventListener('dragstart', function (event) {
        event.dataTransfer.setData('text/plain', '');
...
function setCursorToEnd(element) {
     ...
     document.addEventListener('DOMContentLoaded', function() {
        ...
        dropzone3s.forEach(dropzone3 => {
            ...
            dropzone3.addEventListener('drop', function (event) {
                ...
                if (elementId && event.target.classList.contains('dropzone3')) {
                    ...
                    if (elementTitle) {
                        ...
                        // Append the new element to the dropzone3
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // Existing dropzone3 drop event logic
        const dropzone3 = document.querySelector('.dropzone3'); // Adjust selector as needed
        dropzone3.addEventListener('drop', function (event) {
            event.preventDefault();
            const id = event.dataTransfer.getData('text');
            if (id && event.target.classList.contains('dropzone3')) {
...
//delete button .......................
function clearDropzone3Contents() {
    document.getElementById('dropzone3Contents').innerHTML = '';
}
//sounds for elements......................
