// parameters for elements (dropzone4, elements)
const draggables = document.querySelectorAll('.draggable');
const dropzone4s = document.querySelectorAll('.dropzone4');
let selectedDropzone4 = null;
let dropzone4Count = 1; // Start with 1 because we have a default dropzone4
const MAX_DROPZONE4S = 3;
let labelCounter = 0;
...
// Resizable UI page elements
            ...
            .dropzone4 {
                position: relative;
                border-radius:20px;
                border:5px solid #333; /* Ensure buttons are positioned relative to the dropzone4 */
            }
           .resize-handle { 
                position: absolute; 
                bottom: 0; 
            ...
            .selected-dropzone4 { background-color: #f0f0f0; }
                  .resizable {
            position: relative;
...
dropzone4s.forEach(dropzone4 => {
    ...
    dropzone4.addEventListener('drop', function (event) {
        ...
        if (id && event.target.classList.contains('dropzone4')) {
        }
    });
    dropzone4.addEventListener('click', function () {
        if (selectedDropzone4) {
            selectedDropzone4.classList.remove('selected-dropzone4');
        }
        selectedDropzone4 = dropzone4;
        dropzone4.classList.add('selected-dropzone4');
    });
});
//dropzone4 eventlister ends here.......................................
// Add click event listener to the draggable elements
const draggableElements = document.querySelectorAll('.draggable');
draggableElements.forEach(element => {
    element.addEventListener('click', function () {
        if (selectedDropzone4) {
            const dropzone4Rect = selectedDropzone4.getBoundingClientRect();
            const dropzone4CenterX = dropzone4Rect.width / 2;
            const dropzone4CenterY = dropzone4Rect.height / 2;
            appendElementToDropzone4(element.id, selectedDropzone4, dropzone4CenterX, dropzone4CenterY);
        } else {
            alert('Please select a dropzone4 first.');
        }
    });
});
function appendElementToDropzone4(id, dropzone4, x, y) {
...
function appendElementToDropzone4(id, dropzone4, clientX, clientY) {
    let element;
    switch (id) {
       case 'H1':
        ...
        case 'text':
           ...
           ensureButtons();
            ...
            case 'select':
                ...
                addDeleteButton(element);
                element.style.left = `${clientX - dropzone4.getBoundingClientRect().left}px`;
                element.style.top = `${clientY - dropzone4.getBoundingClientRect().top}px`;
                addMoveButton(element);
                break;
        case 'image':
 ...
 //appending elements to UI page ends here...................................
  //not sure about which part
    ...
    const dropzone4Rect = dropzone4.getBoundingClientRect();
    const offsetX = clientX - dropzone4Rect.left;
    const offsetY = clientY - dropzone4Rect.top;
    if (id === 'image') {
    ...
    if (id === 'image') {
        ...
        element.addEventListener('drop', function (event) {
            event.preventDefault();
            const offsetX = event.clientX - dropzone4Rect.left;
            const offsetY = event.clientY - dropzone4Rect.top;
            element.style.left = `${offsetX}px`;
            element.style.top = `${offsetY}px`;
        });
...
//till here 
//UI page resize
function enableResize(dropzone4) {
    const resizeHandle = dropzone4.querySelector('.resize-handle');
    resizeHandle.addEventListener('mousedown', function (event) {
        event.preventDefault();
        event.stopPropagation();
        const initialHeight = dropzone4.offsetHeight;
        const initialY = event.clientY;
        function onMouseMove(event) {
            const height = initialHeight + (event.clientY - initialY);
            dropzone4.style.height = `${Math.max(50, height)}px`; // Set a minimum height of 50px
        }
        function onMouseUp() {
            document.removeEventListener('mousemove', onMouseMove);
            document.removeEventListener('mouseup', onMouseUp);
...
//Initialize default dropzone4 (when multiple dropzone4s were there)
function initializeDefaultDropzone4() {
    const defaultDropzone4 = document.getElementById('dropzone4');
    const resizeHandle = document.createElement('div');
    resizeHandle.classList.add('resize-handle');
    defaultDropzone4.appendChild(resizeHandle);
    enableResize(defaultDropzone4);
}
//Initialize default dropzone4 (when multiple dropzone4s were there) ends here..........................................
//share button handle function
function handleSharedDropzone4() {
    const urlParams = new URLSearchParams(window.location.search);
    const sharedDropzone4Id = urlParams.get('dropzone4');
    if (sharedDropzone4Id) {
        deserializeDropzone4(sharedDropzone4Id);
    }
}
// Call this function when the page loads
document.addEventListener('DOMContentLoaded', handleSharedDropzone4);
...
const dropzone4BgColorInput = document.getElementById('dropzone4BgColorInput');
...
applyButton.addEventListener('click', function () {
    if (selectedDropzone4) {
        ...
        const dropzone4BgColor = dropzone4BgColorInput.value;
        ...
        // Apply dropzone4 styles
        if (dropzone4BgColor) {
            selectedDropzone4.style.backgroundColor = dropzone4BgColor;
        }
        ...
        if (file) {
            ...
            reader.onload = function (e) {
                ...
                selectedDropzone4.style.backgroundImage = `url(${imageUrl})`;
                selectedDropzone4.style.backgroundSize = 'cover'; 
                selectedDropzone4.style.backgroundPosition = 'center'; 
            };
            reader.readAsDataURL(file);
        }

        // Apply selected element styles if an element is selected within the dropzone4
        const selectedElement = selectedDropzone4.querySelector('.dropped-element.selected');
        if (selectedElement) {
...
// Event listener for selecting elements within the dropzone4
...
function updateLabelDropdown() {
    ...
    const labels = document.querySelectorAll('.dropzone4 label');
...
function downloadTableImage() {
    ...
    if (!dropzone4) {
        console.error('Dropzone4 not found.');
        return;
    }

    html2canvas(dropzone4).then(canvas => {
        ...
        logo.onload = () => {
            ...
            link.href = imgData;
            link.download = 'dropzone4-image.png';
...
document.addEventListener('DOMContentLoaded', () => {
    restoreDropZone4();
    document.querySelector('.save-btn').addEventListener('click', freezeDropZone4);
    document.querySelector('.edit-btn').addEventListener('click', unfreezeDropZone4);
    document.querySelector('.share-btn').addEventListener('click', shareDropZone4);
    // Attach the delete function to the delete button
    document.querySelector('.delete-btn').addEventListener('click', clearDropzone4AndStorage);
});
function generateUniqueId() {
    return 'id-' + Math.random().toString(36).substr(2, 16);
}
function freezeDropZone4() {
    const dropzone4 = document.querySelector('.dropzone4');
    const elementsToSave = Array.from(dropzone4.children).map(element => ({
        id: element.id || generateUniqueId(),
        innerHTML: element.innerHTML,
        classList: Array.from(element.classList),
        style: element.getAttribute('style')
    }));
    localStorage.setItem('dropzone4BgColor', dropzone4.style.backgroundColor);
    localStorage.setItem('dropzone4BgImage', dropzone4.style.backgroundImage); // Save background image URL
    localStorage.setItem('dropzone4Contents', JSON.stringify(elementsToSave));
    Array.from(dropzone4.children).forEach(element => {
...
//edit button
function restoreDropZone4() {
    const dropzone4 = document.querySelector('.dropzone4');
    const savedElements = JSON.parse(localStorage.getItem('dropzone4Contents'));
    const savedBgColor = localStorage.getItem('dropzone4BgColor');
    const savedBgImage = localStorage.getItem('dropzone4BgImage'); // Get background image URL
    if (savedBgColor) {
        dropzone4.style.backgroundColor = savedBgColor;
    }
    if (savedBgImage) {
        dropzone4.style.backgroundImage = savedBgImage; // Apply background image URL
        dropzone4.style.backgroundSize = 'cover'; 
        dropzone4.style.backgroundPosition = 'center'; 
    }
    if (savedElements) {
        dropzone4.innerHTML = ''; // Clear existing content
...
// delete dropzone4
function clearDropzone4AndStorage() {
    const dropzone4 = document.querySelector('.dropzone4');
    if (dropzone4) {
        // Clear the contents of the dropzone4
        dropzone4.innerHTML = '';
        // Clear localStorage
        localStorage.removeItem('dropzone4BgColor');
        localStorage.removeItem('dropzone4Contents');
        localStorage.removeItem('dropzone4BgImage');
        console.log('Dropzone4 cleared and storage reset.');
    } else {
        console.log('Dropzone4 element not found.');
    }
}
// delete dropzone4 ends here.................................

//share button
function shareDropZone4() {
    const dropzone4Content = localStorage.getItem('dropzone4Contents');
    if (!dropzone4Content) {
        console.error('Nothing to share. Save the drop zone first.');
        return;
    }
    const dropzone4Id = 'unique-dropzone4-id';
    localStorage.setItem(dropzone4Id, dropzone4Content);
    const shareableLink = `${window.location.origin}${window.location.pathname}?dropzone4=${dropzone4Id}`;
    console.log('Shareable link:', shareableLink);
    alert(`Shareable link: ${shareableLink}`);
}
function serializeDropzone4(dropzone4) {
    const content = {
        style: dropzone4.getAttribute('style'),
        className: dropzone4.className,
        elements: []
    };
    dropzone4.querySelectorAll('.dropped-element').forEach(element => {
        let elementData = {
...
function generateUniqueId() {
    return 'dropzone4_' + Math.random().toString(36).substr(2, 9);
}
function reattachEventListeners(element) {
    element.addEventListener('dragstart', function (event) {
        event.dataTransfer.setData('text/plain', '');
...
function setCursorToEnd(element) {
     ...
     document.addEventListener('DOMContentLoaded', function() {
        ...
        dropzone4s.forEach(dropzone4 => {
            ...
            dropzone4.addEventListener('drop', function (event) {
                ...
                if (elementId && event.target.classList.contains('dropzone4')) {
                    ...
                    if (elementTitle) {
                        ...
                        // Append the new element to the dropzone4
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // Existing dropzone4 drop event logic
        const dropzone4 = document.querySelector('.dropzone4'); // Adjust selector as needed
        dropzone4.addEventListener('drop', function (event) {
            event.preventDefault();
            const id = event.dataTransfer.getData('text');
            if (id && event.target.classList.contains('dropzone4')) {
...
//delete button .......................
function clearDropzone4Contents() {
    document.getElementById('dropzone4Contents').innerHTML = '';
}
//sounds for elements......................
