// parameters for elements (dropzone2, elements)
const draggables = document.querySelectorAll('.draggable');
const dropzone2s = document.querySelectorAll('.dropzone2');
let selectedDropzone2 = null;
let dropzone2Count = 1; // Start with 1 because we have a default dropzone2
const MAX_DROPZONE2S = 3;
let labelCounter = 0;
...
// Resizable UI page elements
            ...
            .dropzone2 {
                position: relative;
                border-radius:20px;
                border:5px solid #333; /* Ensure buttons are positioned relative to the dropzone2 */
            }
           .resize-handle { 
                position: absolute; 
                bottom: 0; 
            ...
            .selected-dropzone2 { background-color: #f0f0f0; }
                  .resizable {
            position: relative;
...
dropzone2s.forEach(dropzone2 => {
    ...
    dropzone2.addEventListener('drop', function (event) {
        ...
        if (id && event.target.classList.contains('dropzone2')) {
        }
    });
    dropzone2.addEventListener('click', function () {
        if (selectedDropzone2) {
            selectedDropzone2.classList.remove('selected-dropzone2');
        }
        selectedDropzone2 = dropzone2;
        dropzone2.classList.add('selected-dropzone2');
    });
});
//dropzone2 eventlister ends here.......................................
// Add click event listener to the draggable elements
const draggableElements = document.querySelectorAll('.draggable');
draggableElements.forEach(element => {
    element.addEventListener('click', function () {
        if (selectedDropzone2) {
            const dropzone2Rect = selectedDropzone2.getBoundingClientRect();
            const dropzone2CenterX = dropzone2Rect.width / 2;
            const dropzone2CenterY = dropzone2Rect.height / 2;
            appendElementToDropzone2(element.id, selectedDropzone2, dropzone2CenterX, dropzone2CenterY);
        } else {
            alert('Please select a dropzone2 first.');
        }
    });
});
function appendElementToDropzone2(id, dropzone2, x, y) {
...
function appendElementToDropzone2(id, dropzone2, clientX, clientY) {
    let element;
    switch (id) {
       case 'H1':
        ...
        case 'text':
           ...
           ensureButtons();
            ...
            case 'select':
                ...
                addDeleteButton(element);
                element.style.left = `${clientX - dropzone2.getBoundingClientRect().left}px`;
                element.style.top = `${clientY - dropzone2.getBoundingClientRect().top}px`;
                addMoveButton(element);
                break;
        case 'image':
 ...
 //appending elements to UI page ends here...................................
  //not sure about which part
    ...
    const dropzone2Rect = dropzone2.getBoundingClientRect();
    const offsetX = clientX - dropzone2Rect.left;
    const offsetY = clientY - dropzone2Rect.top;
    if (id === 'image') {
    ...
    if (id === 'image') {
        ...
        element.addEventListener('drop', function (event) {
            event.preventDefault();
            const offsetX = event.clientX - dropzone2Rect.left;
            const offsetY = event.clientY - dropzone2Rect.top;
            element.style.left = `${offsetX}px`;
            element.style.top = `${offsetY}px`;
        });
...
//till here 
//UI page resize
function enableResize(dropzone2) {
    const resizeHandle = dropzone2.querySelector('.resize-handle');
    resizeHandle.addEventListener('mousedown', function (event) {
        event.preventDefault();
        event.stopPropagation();
        const initialHeight = dropzone2.offsetHeight;
        const initialY = event.clientY;
        function onMouseMove(event) {
            const height = initialHeight + (event.clientY - initialY);
            dropzone2.style.height = `${Math.max(50, height)}px`; // Set a minimum height of 50px
        }
        function onMouseUp() {
            document.removeEventListener('mousemove', onMouseMove);
            document.removeEventListener('mouseup', onMouseUp);
...
//Initialize default dropzone2 (when multiple dropzone2s were there)
function initializeDefaultDropzone2() {
    const defaultDropzone2 = document.getElementById('dropzone2');
    const resizeHandle = document.createElement('div');
    resizeHandle.classList.add('resize-handle');
    defaultDropzone2.appendChild(resizeHandle);
    enableResize(defaultDropzone2);
}
//Initialize default dropzone2 (when multiple dropzone2s were there) ends here..........................................
//share button handle function
function handleSharedDropzone2() {
    const urlParams = new URLSearchParams(window.location.search);
    const sharedDropzone2Id = urlParams.get('dropzone2');
    if (sharedDropzone2Id) {
        deserializeDropzone2(sharedDropzone2Id);
    }
}
// Call this function when the page loads
document.addEventListener('DOMContentLoaded', handleSharedDropzone2);
...
const dropzone2BgColorInput = document.getElementById('dropzone2BgColorInput');
...
applyButton.addEventListener('click', function () {
    if (selectedDropzone2) {
        ...
        const dropzone2BgColor = dropzone2BgColorInput.value;
        ...
        // Apply dropzone2 styles
        if (dropzone2BgColor) {
            selectedDropzone2.style.backgroundColor = dropzone2BgColor;
        }
        ...
        if (file) {
            ...
            reader.onload = function (e) {
                ...
                selectedDropzone2.style.backgroundImage = `url(${imageUrl})`;
                selectedDropzone2.style.backgroundSize = 'cover'; 
                selectedDropzone2.style.backgroundPosition = 'center'; 
            };
            reader.readAsDataURL(file);
        }

        // Apply selected element styles if an element is selected within the dropzone2
        const selectedElement = selectedDropzone2.querySelector('.dropped-element.selected');
        if (selectedElement) {
...
// Event listener for selecting elements within the dropzone2
...
function updateLabelDropdown() {
    ...
    const labels = document.querySelectorAll('.dropzone2 label');
...
function downloadTableImage() {
    ...
    if (!dropzone2) {
        console.error('Dropzone2 not found.');
        return;
    }

    html2canvas(dropzone2).then(canvas => {
        ...
        logo.onload = () => {
            ...
            link.href = imgData;
            link.download = 'dropzone2-image.png';
...
document.addEventListener('DOMContentLoaded', () => {
    restoreDropZone2();
    document.querySelector('.save-btn').addEventListener('click', freezeDropZone2);
    document.querySelector('.edit-btn').addEventListener('click', unfreezeDropZone2);
    document.querySelector('.share-btn').addEventListener('click', shareDropZone2);
    // Attach the delete function to the delete button
    document.querySelector('.delete-btn').addEventListener('click', clearDropzone2AndStorage);
});
function generateUniqueId() {
    return 'id-' + Math.random().toString(36).substr(2, 16);
}
function freezeDropZone2() {
    const dropzone2 = document.querySelector('.dropzone2');
    const elementsToSave = Array.from(dropzone2.children).map(element => ({
        id: element.id || generateUniqueId(),
        innerHTML: element.innerHTML,
        classList: Array.from(element.classList),
        style: element.getAttribute('style')
    }));
    localStorage.setItem('dropzone2BgColor', dropzone2.style.backgroundColor);
    localStorage.setItem('dropzone2BgImage', dropzone2.style.backgroundImage); // Save background image URL
    localStorage.setItem('dropzone2Contents', JSON.stringify(elementsToSave));
    Array.from(dropzone2.children).forEach(element => {
...
//edit button
function restoreDropZone2() {
    const dropzone2 = document.querySelector('.dropzone2');
    const savedElements = JSON.parse(localStorage.getItem('dropzone2Contents'));
    const savedBgColor = localStorage.getItem('dropzone2BgColor');
    const savedBgImage = localStorage.getItem('dropzone2BgImage'); // Get background image URL
    if (savedBgColor) {
        dropzone2.style.backgroundColor = savedBgColor;
    }
    if (savedBgImage) {
        dropzone2.style.backgroundImage = savedBgImage; // Apply background image URL
        dropzone2.style.backgroundSize = 'cover'; 
        dropzone2.style.backgroundPosition = 'center'; 
    }
    if (savedElements) {
        dropzone2.innerHTML = ''; // Clear existing content
...
// delete dropzone2
function clearDropzone2AndStorage() {
    const dropzone2 = document.querySelector('.dropzone2');
    if (dropzone2) {
        // Clear the contents of the dropzone2
        dropzone2.innerHTML = '';
        // Clear localStorage
        localStorage.removeItem('dropzone2BgColor');
        localStorage.removeItem('dropzone2Contents');
        localStorage.removeItem('dropzone2BgImage');
        console.log('Dropzone2 cleared and storage reset.');
    } else {
        console.log('Dropzone2 element not found.');
    }
}
// delete dropzone2 ends here.................................

//share button
function shareDropZone2() {
    const dropzone2Content = localStorage.getItem('dropzone2Contents');
    if (!dropzone2Content) {
        console.error('Nothing to share. Save the drop zone first.');
        return;
    }
    const dropzone2Id = 'unique-dropzone2-id';
    localStorage.setItem(dropzone2Id, dropzone2Content);
    const shareableLink = `${window.location.origin}${window.location.pathname}?dropzone2=${dropzone2Id}`;
    console.log('Shareable link:', shareableLink);
    alert(`Shareable link: ${shareableLink}`);
}
function serializeDropzone2(dropzone2) {
    const content = {
        style: dropzone2.getAttribute('style'),
        className: dropzone2.className,
        elements: []
    };
    dropzone2.querySelectorAll('.dropped-element').forEach(element => {
        let elementData = {
...
function generateUniqueId() {
    return 'dropzone2_' + Math.random().toString(36).substr(2, 9);
}
function reattachEventListeners(element) {
    element.addEventListener('dragstart', function (event) {
        event.dataTransfer.setData('text/plain', '');
...
function setCursorToEnd(element) {
     ...
     document.addEventListener('DOMContentLoaded', function() {
        ...
        dropzone2s.forEach(dropzone2 => {
            ...
            dropzone2.addEventListener('drop', function (event) {
                ...
                if (elementId && event.target.classList.contains('dropzone2')) {
                    ...
                    if (elementTitle) {
                        ...
                        // Append the new element to the dropzone2
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // Existing dropzone2 drop event logic
        const dropzone2 = document.querySelector('.dropzone2'); // Adjust selector as needed
        dropzone2.addEventListener('drop', function (event) {
            event.preventDefault();
            const id = event.dataTransfer.getData('text');
            if (id && event.target.classList.contains('dropzone2')) {
...
//delete button .......................
function clearDropzone2Contents() {
    document.getElementById('dropzone2Contents').innerHTML = '';
}
//sounds for elements......................
