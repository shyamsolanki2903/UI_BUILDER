// parameters for elements (dropzone6, elements)
const draggables = document.querySelectorAll('.draggable');
const dropzone6s = document.querySelectorAll('.dropzone6');
let selectedDropzone6 = null;
let dropzone6Count = 1; // Start with 1 because we have a default dropzone6
const MAX_DROPZONE6S = 3;
let labelCounter = 0;
...
// Resizable UI page elements
            ...
            .dropzone6 {
                position: relative;
                border-radius:20px;
                border:5px solid #333; /* Ensure buttons are positioned relative to the dropzone6 */
            }
           .resize-handle { 
                position: absolute; 
                bottom: 0; 
            ...
            .selected-dropzone6 { background-color: #f0f0f0; }
                  .resizable {
            position: relative;
...
// dropzone6s 
dropzone6s.forEach(dropzone6 => {
    dropzone6.addEventListener('dragover', function (event) {
        event.preventDefault();
    });
    dropzone6.addEventListener('drop', function (event) {
        event.preventDefault();
        const id = event.dataTransfer.getData('text');
        if (id && event.target.classList.contains('dropzone6')) {
        }
    });
    dropzone6.addEventListener('click', function () {
        if (selectedDropzone6) {
            selectedDropzone6.classList.remove('selected-dropzone6');
        }
        selectedDropzone6 = dropzone6;
        dropzone6.classList.add('selected-dropzone6');
    });
});
//dropzone6 eventlister ends here.......................................
// Add click event listener to the draggable elements
const draggableElements = document.querySelectorAll('.draggable');
draggableElements.forEach(element => {
    element.addEventListener('click', function () {
        if (selectedDropzone6) {
            const dropzone6Rect = selectedDropzone6.getBoundingClientRect();
            const dropzone6CenterX = dropzone6Rect.width / 2;
            const dropzone6CenterY = dropzone6Rect.height / 2;
            appendElementToDropzone6(element.id, selectedDropzone6, dropzone6CenterX, dropzone6CenterY);
        } else {
            alert('Please select a dropzone6 first.');
        }
    });
});
function appendElementToDropzone6(id, dropzone6, x, y) {
...
function appendElementToDropzone6(id, dropzone6, clientX, clientY) {
    let element;
    switch (id) {
       case 'H1':
        ...
        case 'text':
           ...
           ensureButtons();
            ...
            case 'select':
                ...
                addDeleteButton(element);
                element.style.left = `${clientX - dropzone6.getBoundingClientRect().left}px`;
                element.style.top = `${clientY - dropzone6.getBoundingClientRect().top}px`;
                addMoveButton(element);
                break;
        case 'image':
 ...
 //appending elements to UI page ends here...................................
  //not sure about which part
    ...
    const dropzone6Rect = dropzone6.getBoundingClientRect();
    const offsetX = clientX - dropzone6Rect.left;
    const offsetY = clientY - dropzone6Rect.top;
    if (id === 'image') {
    ...
    if (id === 'image') {
        ...
        element.addEventListener('drop', function (event) {
            event.preventDefault();
            const offsetX = event.clientX - dropzone6Rect.left;
            const offsetY = event.clientY - dropzone6Rect.top;
            element.style.left = `${offsetX}px`;
            element.style.top = `${offsetY}px`;
        });
...
//till here 
//UI page resize
function enableResize(dropzone6) {
    const resizeHandle = dropzone6.querySelector('.resize-handle');
    resizeHandle.addEventListener('mousedown', function (event) {
        event.preventDefault();
        event.stopPropagation();
        const initialHeight = dropzone6.offsetHeight;
        const initialY = event.clientY;
        function onMouseMove(event) {
            const height = initialHeight + (event.clientY - initialY);
            dropzone6.style.height = `${Math.max(50, height)}px`; // Set a minimum height of 50px
        }
        function onMouseUp() {
            document.removeEventListener('mousemove', onMouseMove);
            document.removeEventListener('mouseup', onMouseUp);
...
//Initialize default dropzone6 (when multiple dropzone6s were there)
function initializeDefaultDropzone6() {
    const defaultDropzone6 = document.getElementById('dropzone6');
    const resizeHandle = document.createElement('div');
    resizeHandle.classList.add('resize-handle');
    defaultDropzone6.appendChild(resizeHandle);
    enableResize(defaultDropzone6);
}
//Initialize default dropzone6 (when multiple dropzone6s were there) ends here..........................................
//share button handle function
function handleSharedDropzone6() {
    const urlParams = new URLSearchParams(window.location.search);
    const sharedDropzone6Id = urlParams.get('dropzone6');
    if (sharedDropzone6Id) {
        deserializeDropzone6(sharedDropzone6Id);
    }
}
// Call this function when the page loads
document.addEventListener('DOMContentLoaded', handleSharedDropzone6);
...
const dropzone6BgColorInput = document.getElementById('dropzone6BgColorInput');
...
applyButton.addEventListener('click', function () {
    if (selectedDropzone6) {
        ...
        const dropzone6BgColor = dropzone6BgColorInput.value;
        ...
        // Apply dropzone6 styles
        if (dropzone6BgColor) {
            selectedDropzone6.style.backgroundColor = dropzone6BgColor;
        }
        ...
        if (file) {
            ...
            reader.onload = function (e) {
                ...
                selectedDropzone6.style.backgroundImage = `url(${imageUrl})`;
                selectedDropzone6.style.backgroundSize = 'cover'; 
                selectedDropzone6.style.backgroundPosition = 'center'; 
            };
            reader.readAsDataURL(file);
        }

        // Apply selected element styles if an element is selected within the dropzone6
        const selectedElement = selectedDropzone6.querySelector('.dropped-element.selected');
        if (selectedElement) {
...
// Event listener for selecting elements within the dropzone6
...
function updateLabelDropdown() {
    ...
    const labels = document.querySelectorAll('.dropzone6 label');
...
function downloadTableImage() {
    ...
    if (!dropzone6) {
        console.error('Dropzone6 not found.');
        return;
    }

    html2canvas(dropzone6).then(canvas => {
        ...
        logo.onload = () => {
            ...
            link.href = imgData;
            link.download = 'dropzone6-image.png';
...
document.addEventListener('DOMContentLoaded', () => {
    restoreDropZone6();
    document.querySelector('.save-btn').addEventListener('click', freezeDropZone6);
    document.querySelector('.edit-btn').addEventListener('click', unfreezeDropZone6);
    document.querySelector('.share-btn').addEventListener('click', shareDropZone6);
    // Attach the delete function to the delete button
    document.querySelector('.delete-btn').addEventListener('click', clearDropzone6AndStorage);
});
function generateUniqueId() {
    return 'id-' + Math.random().toString(36).substr(2, 16);
}
function freezeDropZone6() {
    const dropzone6 = document.querySelector('.dropzone6');
    const elementsToSave = Array.from(dropzone6.children).map(element => ({
        id: element.id || generateUniqueId(),
        innerHTML: element.innerHTML,
        classList: Array.from(element.classList),
        style: element.getAttribute('style')
    }));
    localStorage.setItem('dropzone6BgColor', dropzone6.style.backgroundColor);
    localStorage.setItem('dropzone6BgImage', dropzone6.style.backgroundImage); // Save background image URL
    localStorage.setItem('dropzone6Contents', JSON.stringify(elementsToSave));
    Array.from(dropzone6.children).forEach(element => {
...
//edit button
function restoreDropZone6() {
    const dropzone6 = document.querySelector('.dropzone6');
    const savedElements = JSON.parse(localStorage.getItem('dropzone6Contents'));
    const savedBgColor = localStorage.getItem('dropzone6BgColor');
    const savedBgImage = localStorage.getItem('dropzone6BgImage'); // Get background image URL
    if (savedBgColor) {
        dropzone6.style.backgroundColor = savedBgColor;
    }
    if (savedBgImage) {
        dropzone6.style.backgroundImage = savedBgImage; // Apply background image URL
        dropzone6.style.backgroundSize = 'cover'; 
        dropzone6.style.backgroundPosition = 'center'; 
    }
    if (savedElements) {
        dropzone6.innerHTML = ''; // Clear existing content
...
// delete dropzone6
function clearDropzone6AndStorage() {
    const dropzone6 = document.querySelector('.dropzone6');
    if (dropzone6) {
        // Clear the contents of the dropzone6
        dropzone6.innerHTML = '';
        // Clear localStorage
        localStorage.removeItem('dropzone6BgColor');
        localStorage.removeItem('dropzone6Contents');
        localStorage.removeItem('dropzone6BgImage');
        console.log('Dropzone6 cleared and storage reset.');
    } else {
        console.log('Dropzone6 element not found.');
    }
}
// delete dropzone6 ends here.................................

//share button
function shareDropZone6() {
    const dropzone6Content = localStorage.getItem('dropzone6Contents');
    if (!dropzone6Content) {
        console.error('Nothing to share. Save the drop zone first.');
        return;
    }
    const dropzone6Id = 'unique-dropzone6-id';
    localStorage.setItem(dropzone6Id, dropzone6Content);
    const shareableLink = `${window.location.origin}${window.location.pathname}?dropzone6=${dropzone6Id}`;
    console.log('Shareable link:', shareableLink);
    alert(`Shareable link: ${shareableLink}`);
}
function serializeDropzone6(dropzone6) {
    const content = {
        style: dropzone6.getAttribute('style'),
        className: dropzone6.className,
        elements: []
    };
    dropzone6.querySelectorAll('.dropped-element').forEach(element => {
        let elementData = {
...
function generateUniqueId() {
    return 'dropzone6_' + Math.random().toString(36).substr(2, 9);
}
function reattachEventListeners(element) {
    element.addEventListener('dragstart', function (event) {
        event.dataTransfer.setData('text/plain', '');
...
function setCursorToEnd(element) {
     ...
     document.addEventListener('DOMContentLoaded', function() {
        ...
        dropzone6s.forEach(dropzone6 => {
            ...
            dropzone6.addEventListener('drop', function (event) {
                ...
                if (elementId && event.target.classList.contains('dropzone6')) {
                    ...
                    if (elementTitle) {
                        ...
                        // Append the new element to the dropzone6
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // Existing dropzone6 drop event logic
        const dropzone6 = document.querySelector('.dropzone6'); // Adjust selector as needed
        dropzone6.addEventListener('drop', function (event) {
            event.preventDefault();
            const id = event.dataTransfer.getData('text');
            if (id && event.target.classList.contains('dropzone6')) {
...
//delete button .......................
function clearDropzone6Contents() {
    document.getElementById('dropzone6Contents').innerHTML = '';
}
//sounds for elements......................
